...
result = df.groupby('Group')['Ext'].value_counts().unstack(fill_value=0)
print(result)
...
if p_value < alpha:
    print("Відхиляємо нульову гіпотезу — існує залежність між групою та додатковими заняттями.")
else:
    print("Приймаємо нульову гіпотезу — зв'язку між групою та додатковими заняттями немає.")
